...
FlexiSpot 昇降デスクを PC から操作しながら、立ち座り習慣を可視化できるブラウザベースのコントローラーです。Chrome / Edge の Web Serial API を使ってデスクに接続し、上下移動、現在高さ表示、プリセット呼び出し、日次・週次の高さログ集計を行えます。
...
- 高さログから、立位時間・座位時間・姿勢切替の目安を表示します
- Chrome / Edge 系ブラウザが必要です
- `HTTPS` または `localhost` で開く必要があります
- 初回接続時にシリアルポートの許可が必要です
- 健康ログは習慣の可視化とリマインド目的であり、医療効果や健康改善を保証するものではありません
...
このアプリは、デスクから受信した高さをもとに、次のような立ち座り習慣の目安を表示します。
...
- 立位目標までの残り時間
- 週次の立位時間、座位時間、切替回数
- CSV / JSON 形式の週次エクスポート

しきい値から推定しているため、ユーザーの実際の姿勢、体調、活動量を医学的に判定するものではありません。体調や治療、運動制限については医療専門家の判断を優先してください。

## 記録される内容と保存先

記録される内容は、デスクから受信した高さ、受信時刻、そこから推定した立位・座位セッション、アプリ設定、プリセット名です。氏名、体重、症状、診断情報などの医療情報は入力・保存しません。

未ログイン時は、これらのデータをこのブラウザの `localStorage` に保存します。Google Sign-In を使った場合のみ、同じデータをユーザーの Firebase / Firestore に同期します。CSV / JSON エクスポートはユーザーがボタンを押したときだけローカルで生成されます。
...
- 現在の高さをブラウザで確認する
- 保存済みプリセットを呼び出す
- その日の高さ履歴、立位時間、座位時間を確認する
- 座りっぱなしの目安を検知して立つきっかけを作る
- 週次の健康ログを CSV / JSON で出力する
- 必要なときだけ診断ログを見る
...
- 健康ログは医療機器としての測定や診断を目的にしていません
- 表示される座位・立位時間はデスク高さからの推定であり、実際の姿勢と一致しない場合があります
- 実機接続は自己責任で行ってください
